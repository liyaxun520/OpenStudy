...
void register_inner_class(JNIEnv *env) {
    int ret = find_class(env, "com/decard/lib/jnistruct/data/DataBean$Inner", &m_inner_block.clazz);
    if (ret != 0) {
...
void register_data_bean_class(JNIEnv *env) {
    ...
    get_field(env, &clazz, "mInner", "Lcom/decard/lib/jnistruct/data/DataBean$Inner;", &m_data_bean_block.inner);
